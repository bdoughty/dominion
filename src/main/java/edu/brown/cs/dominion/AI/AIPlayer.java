...
public class AIPlayer extends Player
{
     ...
     *
     * while (g.getPlayerFromUser(this).getActions() > 0) { int actionLoc =
     * st.playAction(g, this);
     *
     * if (actionLoc == -1) { break; } }
     *
     * g.endActionPhase(this);
     *
     * int buys = g.getPlayerFromUser(this).getBuys(); int money =
     * g.getPlayerFromUser(this).getMoney();
     *
     * List<Integer> toBuy = new LinkedList<>();
     *
     * while (buys > 0) { int buyId = st.buy(money, g, this);
     *
     * if (buyId == -1) { break; }
     *
     * try { toBuy.add(buyId); buys--; money -=
     * g.getBoard().getCostFromId(buyId); } catch (NoPileException npe) {
     * System.out.println(npe.getMessage()); break; } }
     *
     ...
     * "militiadiscard": int toDiscard = st.discard(g, this); if (toDiscard >=
     * 0) { c.getCallback().call(this, true, toDiscard); } break; } } }
     */
