...
public class NeuralNetwork {
  ...
  public NeuralNetwork(int... layerSizes) {
    ...
    for (int i = 0; i < layers.length - 1; i++) {
      layers[i] = new FCLayer(layerSizes[i], layerSizes[i + 1],
          ActivationFunction.RELU);
    }
    layers[layers.length - 1] = new FCLayer(layerSizes[layers.length - 1],
        layerSizes[layers.length], ActivationFunction.LINEAR);
  }
